...
let clickMobileMenu = document.getElementsByClassName("clickMobileMenu");
...
for(let i=0; i<clickMobileMenu.length; i++) {
  clickMobileMenu[i].addEventListener("click",()=>{
...
let cardBody = document.getElementsByClassName("projectHeight"); 

for(let i = 0; i<sizeCard.length; i++) {
  if(sizeCard[i].clientHeight < cardBody[i].clientHeight) {
    let overCardBody = document.getElementsByClassName("projectHeight")[i];
    let seeMore = `<div class= "seeMore" id=${i}>..see more
    </div>` ;  
    overCardBody.innerHTML += seeMore;
    let seeLess = `<div class= "seeLess" id=${i}>..see less
    </div>`;
    overCardBody.innerHTML += seeLess;
  }
};

let seeMoreClick = document.getElementsByClassName("seeMore");

for(let i = 0; i<seeMoreClick.length; i++) {
  seeMoreClick[i].addEventListener("click",(e)=>{
    let catchSeeMore = e.target;
    let catchId = catchSeeMore.id;
    sizeCard[catchId].style.height = "fit-content";
    catchSeeMore.style.display = "none";
    
    let catchSeeLessClick = document.getElementsByClassName("seeLess");
    for(let i = 0; i<catchSeeLessClick.length; i++) {
      catchSeeLessClick[i].addEventListener("click",(e)=>{
        let catchSeeLess = e.target;
        let catchIdSeeLess = catchSeeLess.id;
        sizeCard[catchIdSeeLess].style.height = 330 + "px";
        console.log(catchSeeLess)
        console.log(catchSeeMore)
        catchSeeMore.style.display = "block"; 
      })
    }
  })
}
